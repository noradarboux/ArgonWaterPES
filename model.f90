...
IMPLICIT NONE
REAL(dp) :: y(50), t(50,3), alf(12), beta(8), a(50,13), w(50), eta, A1, A2, B
...
110 FORMAT ('1   NON-LINEAR LEAST SQUARES PROBLEM'//  &
    ' NUMBER OF OBSERVATIONS =',i5, '   NUMBER OF LINEAR PARAMETERS
=',i4//  &
    ' NUMBER OF NONLINEAR PARAMETERS =',i4,  &
    ' NUMBER OF NONVANISHING PARTIAL DERIVATIVES =',i4//  &
    ' NUMBER OF INDEPENDENT VARIABLES =',i4// '    I    T(I)
Y(I)'//)
120 FORMAT (4E20.7)
...
SUBROUTINE ada (lp1, nl, n, nmax, lpp2, iv, a, inc, t, alf, isel, A1, A2, B)
...
REAL (dp), INTENT(IN)   :: t(:,:)        ! t(nmax,iv+1)
REAL (dp), INTENT(IN)   :: alf(:)        ! alf(nl)
INTEGER, INTENT(IN)     :: isel
!local variables
real*8                  :: A1, A2, B     !calculated from the data
INTEGER :: i
...
30 DO  i=1,n
  a(i,1) = A1*t(i,1)*exp(-alf(1)*t(i,1)**2)+B*exp(-alf(2)*t(i,1)**2) &
+A2*t(i,2)*exp(-alp(3)*t(i,2)**2)+B*exp(-alp(4)*t(i,2)**2)-B
...
50 DO  i=1,n
  a(i,2) = -t(i,1)**3*A1*EXP(-alf(1)*t(i,1))
  a(i,3) = -t(i,1)**2*B*EXP(-alf(2)*t(i,1))
  a(i,4) = -t(i,2)**3*A2*EXP(-alf(3)*t(i,2))
  a(i,5) = -t(i,2)**2*B*EXP(-alp(4)*t(i,2))
END DO
